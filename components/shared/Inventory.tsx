...
const Inventory: React.FC<IInventory> = ({ inventory }) => {
  return !inventory || inventory.length === 0 ? (
    ...
    <div className='grid grid-cols-3 md:grid-cols-5 border border-black border-solid border-opacity-50'>
...
const InventoryItem: React.FC<IInventoryItem> = ({ item }) => {
  ...
  return (
    <div className='relative w-full h-24 border border-solid border-black border-opacity-25 shadow-md'>
