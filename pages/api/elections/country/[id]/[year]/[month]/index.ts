...
export default async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  switch (req.method) {
    case 'GET': {
      ...
      let result = await getElection(countryId, year, month);
...
async function getElection(countryId: number, year: number, month: number): Promise<GetElectionResponse> {
  let query = {
  ...
  if (!election)
    return { status_code: 404, payload: { error: 'Election Not Found' } };
